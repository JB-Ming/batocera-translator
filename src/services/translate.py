    ...
    def _get_translator(self):
        ...
        if self._translator is None:
            try:
                ...
                raise ImportError(
                    "請先安裝 googletrans: pip install googletrans==4.0.0-rc1")
...
class TranslateService:
    ...
    def __init__(self, provider: TranslateProvider = TranslateProvider.GOOGLETRANS,
                 api_key: str = '', request_delay: float = 1.0):
        """
        初始化翻譯服務
    ...
    def _get_service(self) -> BaseTranslateService:
        ...
        if self._service is None:
            if self.provider == TranslateProvider.GOOGLETRANS:
                ...
                self._service = DeepLService(
                    self.api_key, request_delay=self.request_delay)
            else:
                # 預設使用免費的 googletrans
                self._service = GoogleTransService(
                    request_delay=self.request_delay)
